...
static void deh_procStrings(DEHFILE *fpin, char *line)
{
	...
	if( devparm )
	{
		...
		logd( "Processing extended string substitution\n" );
	}
    ...
    while (!dehfeof(fpin) && *inbuffer)
    {
        ...
        while (strlen(holdstring) + len > (unsigned int)maxstrlen)
        {
			...
			if( devparm )
			{
				...
				logd( "* increased buffer from to %i for buffer size %i\n", maxstrlen, len );
			}
...
static dboolean deh_procStringSub(char *key, char *lookfor, char *newstring)
{
            ...
            *t = '\0';                                                      // cap off the target string
            ...
            if (devparm)
            {
                ...
                else
                {
					...
					logd( "Assigned \"%.12s%s\" to \"%.12s%s\" at key %s\n", lookfor, ( strlen( lookfor ) > 12 ? "..." : "" ),
						newstring, ( strlen( newstring ) > 12 ? "..." : "" ), deh_strlookup[ i ].lookup );
					logd( "*BEX FORMAT:\n" );
	...
	if( !found && !hacx )
	{
		...
		loge( "The <b>\"%s\"</b> string can't be found.\n", ( key ? key : lookfor ) );
	}
