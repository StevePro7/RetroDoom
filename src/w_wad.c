...
dboolean W_AddFile(char *filename, dboolean automatic)
{
	...
	logd( "%s %s lump%s from %s <b>%s</b>.\n", ( automatic ? "Automatically added" : "Added" ), temp,
		( numlumps - startlump == 1 ? "" : "s" ), ( wadfile->type == IWAD ? "IWAD" : "PWAD" ), wadfile->path );
    free(temp);
        ...
        || M_StringEndsWith(filename, "SIGIL.wad"))
    {
		...
		logd( "<i><b>SIGIL</b></i> is now available to play from the episode menu.\n" );
    }
    else if (M_StringEndsWith(filename, "SIGIL_SHREDS.WAD") || M_StringEndsWith(filename, "SIGIL_SHREDS_COMPAT.wad"))
    {
		...
		logd( "Buckethead's soundtrack will now be used when playing <i><b>SIGIL.</b></i>\n" );
    }
	else if( M_StringEndsWith( filename, "DOOM.WAD" ) )
	{
		...
		logd( "<i><b>E1M4B: Phobos Mission Control</b></i> and <i><b>E1M8B: Tech Gone Bad</b></i>\n"
			"are now available to play using the <b>map</b> CCMD." );
	}
	...
	else if( M_StringEndsWith( filename, "NERVE.WAD" ) )
	{
		...
		logd( "<i><b>No Rest For The Living</b></i> is now available to play from the expansion menu.\n" );
	}
